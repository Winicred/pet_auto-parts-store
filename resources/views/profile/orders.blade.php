          ...
          @foreach($orders as $order)
            ...
            @section('modals')
              ...
              <div class="modal modal-bottom sm:modal-middle fixed z-[100]">
                <div class="modal-box">
                  <h3
                    class="font-bold text-lg">@lang('pages.my_orders.content.modal.title', ['order_number' => $order->id])</h3>

                  <div class="overflow-y-auto max-h-[500px] mt-4">
                    <div class="flex flex-col h-full mx-2">
                      @foreach($order->parts as $product)
                        <div class="flex gap-3 transition-colors duration-250 hover:bg-base-300/70 p-2 cursor-pointer"
                             onclick="window.location.href = '{{ route('car.parts.show', [$product->part->category->model->car->slug, $product->part->category->model->slug, $product->part->category->slug, $product->part->id, $product->part->modification ?? null]) }}'">
                          <div class="flex gap-3 items-center w-full rounded">
                            <div
                              class="aspect-square max-w-[65px] flex items-center justify-center">
                              <img
                                class="max-w-16 h-auto my-auto object-scale-down lazy-image"
                          ...
                          <div class="flex flex-col justify-center">
                                                    <span class="font-semibold flex gap-1">{{ money($product->part->price, 'EUR', true) }}
                                                        <span
                                                          class="text-gray-500 text-sm">(x{{ $product->quantity }})</span>
                                                    </span>
